    ...
    if not mray:
        if ref_data["foul"] == 1: # Penalty kick
            ...
            if ref_data["yellow"]:  # Defensive
                if dp == "direct":
                    lista_r_0 = SendRobotPosition(mray, ref_data, dp, None, 0)
                elif dp == "spin" or dp == "spin-v":
                    lista_r_0 = SendRobotPosition(mray, ref_data, 'spin_spin-v', None, 0)
                lista_r_1 = SendRobotPosition(mray, ref_data, 'spin_spin-v', None, 1) # Center back
                lista_r_2 = SendRobotPosition(mray, ref_data, 'spin_spin-v', None, 2) # Striker
            
            else:  # Ofensive
                #1 está indo a posição, falta verificação de posição correta
                lista_r_0 = SendRobotPosition(mray, ref_data, 'direct_switch', None, 0)
                if op == "direct" or op == "switch":
                    lista_r_1 = SendRobotPosition(mray, ref_data, 'direct_switch', None, 1)
                    lista_r_2 = SendRobotPosition(mray, ref_data, 'direct_switch', None, 2)
                elif op == "spin":
                    r = random.uniform(0, 1) # Generate random number between 0 and 1
                    if r < 0.001: # 0.5 is default value
                        lista_r_1 = SendRobotPosition(mray, ref_data, op, 'if', 1)
                        lista_r_2 = SendRobotPosition(mray, ref_data, op, 'if', 2)
                    else:
                        lista_r_1 = SendRobotPosition(mray, ref_data, op, 'else', 1)
                        lista_r_2 = SendRobotPosition(mray, ref_data, op, 'else', 2)
            #Replace each robot            
        ...
        # TODO FOULS: Revisar as posições futuramente do goalKick
            ...
            if not ref_data["yellow"]:
                lista_r_0 = SendRobotPosition(mray, ref_data, None, None, 0)
                lista_r_1 = SendRobotPosition(mray, ref_data, None, None, 1)
                lista_r_2 = SendRobotPosition(mray, ref_data, None, None, 2)
            else:
                lista_r_0 = SendRobotPosition(mray, ref_data, None, None, 0)
                lista_r_1 = SendRobotPosition(mray, ref_data, None, None, 1)
                if ball.yPos < 65:
                    lista_r_2 = SendRobotPosition(mray, ref_data, 'ball_yPos', None, 2)
            #Replace each robot
            Robot2Position(robot0, ball, robot1, robot2, robotEnemy0, robotEnemy1, robotEnemy2, lista_r_0, lista_r_1, lista_r_2)
    ...
    if mray:    
        if ref_data["foul"] == 4: #Kickoff
            ...
            if not ref_data["yellow"]: # Defensive
                ...
                lista_r_0 = SendRobotPosition(mray, ref_data, None, 0)
                lista_r_1 = SendRobotPosition(mray, ref_data, None, 1)
                lista_r_2 = SendRobotPosition(mray, ref_data, None, 2)
            # Replace each robot
            Robot2Position(robot0, ball, robot1, robot2, robotEnemy0, robotEnemy1, robotEnemy2, lista_r_0, lista_r_1, lista_r_2)
