...
class Strategy:
    ...
    def basic_stg_def(self):
        if not self.mray:
            if self.ball.xPos < 30 and 30 < self.ball.yPos < 110:  # If the ball has inside of defense area
                ...
                action.shoot(self.robot1, self.ball, left_side=not self.mray, friend1=self.robot0, friend2=self.robot2,
                             enemy1=self.robotEnemy0, enemy2=self.robotEnemy1,
                             enemy3=self.robotEnemy2)  # Defender chases ball
                action.screen_out_ball(self.robot0, self.ball, 14, left_side=not self.mray, upper_lim=81,
        ...
        action.screen_out_ball(self.robot2, self.ball, 110, left_side=not self.mray, upper_lim=120,
    ...
    def basic_stg_att(self):
        action.defender_spin(self.robot2, self.ball, left_side=not self.mray, friend1=self.robot0, friend2=self.robot1,
                             enemy1=self.robotEnemy0, enemy2=self.robotEnemy1,
                             enemy3=self.robotEnemy2)  # Attacker behavior
        action.screen_out_ball(self.robot1, self.ball, 60, left_side=not self.mray, upper_lim=120,
    ...
    def stg_def_v2(self):
        if not self.mray:
            if self.ball.xPos < 40 and 30 < self.ball.yPos < 110:  # If the ball has inside of defense area
                ...
                self.two_attackers()
                action.screen_out_ball(self.robot0, self.ball, 16, left_side=not self.mray, upper_lim=84,
                                       lower_lim=42)  # Goalkeeper keeps in goal
    ...
    def penalty_mode_defensive(self):
        action.defender_penalty(self.robot0, self.ball, left_side=not self.mray, friend1=self.robot1,
                                friend2=self.robot2,
                                enemy1=self.robotEnemy0, enemy2=self.robotEnemy1,
                                enemy3=self.robotEnemy2)  # Goalkeeper behaviour in defensive penalty
        action.shoot(self.robot1, self.ball, left_side=not self.mray, friend1=self.robot0, friend2=self.robot2,
    ...
    def penalty_mode_offensive(self):
        ...
        action.shoot(self.robot1, self.ball, left_side=not self.mray, friend1=self.robot0, friend2=self.robot2,
                     enemy1=self.robotEnemy0, enemy2=self.robotEnemy1,
                     enemy3=self.robotEnemy2)  # Defender going to the rebound
        action.attack_penalty(self.robot2, self.ball, leftSide=not self.mray, friend1=self.robot0, friend2=self.robot1,
                              enemy1=self.robotEnemy0, enemy2=self.robotEnemy1,
                              enemy3=self.robotEnemy2)  # Attacker behaviour in penalty
    ...
    def distancia_provisoria(self, objeto1, objeto2):
        return numpy.sqrt(numpy.square(pow(objeto1.xPos - objeto2.xPos, 2) + pow(objeto1.yPos - objeto2.ypos, 2)))

    def close_in_enemy(self):
        #  TODO:Reescrever essa merda o mais rapido possivel, esse eh possivelmente o pior codigo que ja escrevi puta que pariu que coisa horrivel
        inimigos = [self.robotEnemy0, self.robotEnemy1, self.robotEnemy2]
        distancia1 = None
        for i in range(3):
            if distancia1 is None or distancia1 > self.distancia_provisoria(self.robot1, inimigos[i]):
                distancia1 = self.distancia_provisoria(self.robot1, inimigos[i])
                enemy1 = inimigos[i]
        distancia1 = None
        for u in range(3):
            if distancia1 is None or distancia1 > self.distancia_provisoria(self.robot2, inimigos[u]):
                distancia1 = self.distancia_provisoria(self.robot2, inimigos[u])
                enemy2 = inimigos[u]

        action.follower(self.robot1, enemy1, self.ball)
        action.follower(self.robot2, enemy2, self.ball)
