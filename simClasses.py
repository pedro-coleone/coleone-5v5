...
class Robot:
    ...
    def sort_enemies(self):
        n = len(self.enemy_list)
        for i in range(n - 1):
            for u in range(0, n - i - 1):
                if self.dist(self.enemy_list[u]) > self.dist(self.enemy_list[u + 1]):
                    self.enemy_list[u], self.enemy_list[u + 1] = self.enemy_list[u + 1], self.enemy_list[u]
